...
function levelToDuration(level) {
  ...
  if (l.includes('phd')||l.includes('doctorate')) return '3-5 thn';
  if (l.includes('master')||l.includes('postgrad')) return '1.5-2 thn';
  if (l.includes('bachelor')||l.includes('honours')) return '3-4 thn';
  if (l.includes('advanced_diploma')) return '1.5-2 thn';
  if (l.includes('diploma')) return '1-2 thn';
  if (l.includes('certificate')) return '6-12 bln';
  if (l.includes('foundation')||l.includes('grade_12')) return '8-12 bln';
  if (l.includes('english')||l.includes('language')) return '3-12 bln';
  return null;
}

function getProgramDurasi(p) {
  const mn = p.min_length, mx = p.max_length;
  if (mn) {
    const hi = mx || mn;
    if (hi > 18) {
      const a = +(mn/12).toFixed(1), b = +(hi/12).toFixed(1);
      return (a === b ? a : a+'-'+b) + ' thn';
    }
    return (mn === hi ? mn : mn+'-'+hi) + ' bln';
  }
  if (p.duration_months) {
    const d = p.duration_months;
    return d > 18 ? +(d/12).toFixed(1)+' thn' : d+' bln';
  }
  return levelToDuration(p.level);
}
...
async function openDetail(id) {
  ...
  renderAcademic(data.data_json, data);
  renderBiaya(data.data_json, data);
...
}function renderAcademic(dj, _uni) {
...
function renderRequirements(dj) {
  ...
  if (abReqs.length) {
    html += `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:10px 14px;margin-bottom:12px;font-size:11px;color:#92400e">
      ⚠️ <strong>Perhatian:</strong> Data persyaratan ini diambil dari <strong>1 program sampel</strong> saat sync ApplyBoard, bukan per-program individual.
      Persyaratan aktual masing-masing program bisa berbeda — selalu verifikasi langsung di halaman program ApplyBoard.
    </div>`;
